...
@implementation UIImage (ZEImage)
...
- (UIImage *)cropToCircleInRect:(CGRect)rect
{
    ...
    CGRect imgaeRect = CGRectMake(0, 0, self.size.width, self.size.height);
    UIGraphicsBeginImageContextWithOptions(self.size, YES, 0.0);
    {
        CGContextRef context = UIGraphicsGetCurrentContext();
        [[UIColor blackColor] setFill];
        UIRectFill(imgaeRect);
    ...
    {
        CGContextClipToMask(UIGraphicsGetCurrentContext(), imgaeRect, mask.CGImage);
        [self drawAtPoint:CGPointZero];
    }
    UIImage *maskedImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    
    CGRect croppedRect = rect;
    
//    croppedRect.origin.y = imgaeRect.size.height - rect.origin.y;
    croppedRect.origin.x *= [[UIScreen mainScreen] scale];
    croppedRect.origin.y *= [[UIScreen mainScreen] scale];
    croppedRect.size.width *= [[UIScreen mainScreen] scale];
    croppedRect.size.height *= [[UIScreen mainScreen] scale];
    
    maskedImage = [maskedImage croppedImage:croppedRect];
     ...
     interpolationQuality:(CGInterpolationQuality)quality
{
    ...
    CGContextRef bitmap = CGBitmapContextCreate(NULL,
                                                ...
                                                CGImageGetBitmapInfo(imageRef)); //[To-Do]uncertain about this parameter
